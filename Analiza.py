...
data = {
    ...
    "Sudden death": [
        round(np.mean(RMSSD_list_heart_failure),3),
        round(np.mean(SDNN_list_heart_failure), 3),
        round(np.mean(pNN50_list_heart_failure), 3)
    ],
    "AF": [
        round(np.mean(RMSSD_list_heart_AF), 3),
        round( np.mean(SDNN_list_heart_AF), 3),
        round( np.mean(pNN50_list_heart_AF), 3),
    ],
    "CHF": [
        round(np.mean(RMSSD_list_heart), 3),
        round(np.mean(SDNN_list_heart), 3),
        round( np.mean(pNN50_list_heart), 3)
    ],
    "Zdrowi": [
        round(np.mean(RMSSD_list), 3),
        round(np.mean(SDNN_list), 3),
        round( np.mean(pNN50_list), 3)
    ]
}
...
pdfs = ['table.pdf', 'table1.pdf']

merger = PdfMerger()

for pdf in pdfs:
    merger.append( pdf,pages=0)
